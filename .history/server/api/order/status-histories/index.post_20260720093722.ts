      ...
      `UPDATE tb_shopping_orders
       SET order_status = $1,
           order_confirmed_at = CASE
             WHEN $1 = 'confirmed' THEN COALESCE(order_confirmed_at, NOW())
             ELSE order_confirmed_at
           END,
           order_shipped_at = CASE
             WHEN $1 = 'shipped' THEN COALESCE(order_shipped_at, NOW())
             ELSE order_shipped_at
           END,
           order_completed_at = CASE
             WHEN $1 = 'completed' THEN COALESCE(order_completed_at, NOW())
             ELSE order_completed_at
           END,
           order_canceled_at = CASE
             WHEN $1 = 'canceled' THEN COALESCE(order_canceled_at, NOW())
