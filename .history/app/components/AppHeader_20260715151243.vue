<template>
  ...
  <header class="shrink-0 border-b border-base-300 bg-base-100">
    <div
      ...
      :class="isTopBarVisible ? 'max-h-9 translate-y-0 opacity-100' : 'max-h-0 -translate-y-2 opacity-0 pointer-events-none'"
    >
      <div class="mx-auto flex h-9 w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <NuxtLink
          v-if="isLoggedIn"
          to="/products"
        ...
        <span v-else class="text-xs text-primary-content/80">Finleland Plaza</span>
      </div>
    </div>

    <div class="mx-auto flex h-18 w-full max-w-7xl items-center justify-between gap-3 px-4 sm:px-6 lg:h-20 lg:px-8">
      <NuxtLink to="/" class="shrink-0" aria-label="หน้าแรก">
        <img
          src="@/assets/images/logo.png"
          alt="Finleland Plaza"
          class="h-12 w-auto object-contain sm:h-14 lg:h-16"
        />
      </NuxtLink>
      ...
      <nav class="hidden items-center gap-5 xl:flex" aria-label="เมนูหลัก">
        ...
        <NuxtLink
          ...
          :class="route.path === '/shopping-basket' ? 'font-semibold text-primary' : ''"
        >
          <Icon name="lucide:shopping-cart" size="16" />
          ตะกร้าสินค้า
          <span
            class="badge badge-sm rounded-full text-xs"
            :class="route.path === '/shopping-basket' ? 'badge-primary' : 'badge-secondary'"
          >
            {{ itemCount }}
          </span>
        </NuxtLink>
      ...
      <div class="flex min-w-0 shrink-0 items-center gap-1.5 sm:gap-2">
        <template v-if="isLoggedIn">
          ...
          >
            ...
            <div class="min-w-0 text-left">
              ...
              <p class="hidden max-w-40 truncate text-[11px] text-base-content/55 2xl:block">
                {{ user?.email }}
              </p>
            </div>
          </NuxtLink>
        ...
        <template v-else>
          <button class="btn btn-outline btn-primary btn-xs sm:btn-sm" @click="onSignIn">
            <Icon name="lucide:log-in" size="15" />
            <span class="hidden sm:inline">เข้าสู่ระบบ</span>
          </button>
          <button class="btn btn-primary btn-xs sm:btn-sm" @click="onSignUp">
  ...
  <div class="fab fixed bottom-5 right-5 z-50 xl:hidden">
    ...
    >
      ...
      <span class="badge badge-secondary badge-xs absolute -right-1 -top-1 rounded-full">
        {{ itemCount }}
      </span>
    </NuxtLink>
    <NuxtLink
...
const menus = [
  ...
  { path: "/how-to-order", title: "วิธีการสั่งซื้อ", icon: "lucide:circle-help" },
];
