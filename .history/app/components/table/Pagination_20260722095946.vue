    ...
    <div class="order-1 flex min-w-0 justify-center sm:order-2">
      <div class="flex max-w-full items-center gap-1 overflow-x-auto px-1 py-1">
        ...
        <template v-for="(value, index) in paginationItems" :key="`${value}-${index}`">
          <button
            v-if="value !== '...'"
            class="text-primary! btn btn-square btn-xs border-base-content/10 bg-base-100 shadow-sm transition-transform hover:-translate-y-0.5 sm:btn-sm"
            :class="{
              'btn-primary pointer-events-none shadow-primary/20': pageModel === value,
