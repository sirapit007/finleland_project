<template>
  <div
    class="card border border-base-content/20 shadow-sm hover:scale-105 hover:bg-base-200 hover:shodow-lg transition cursor-pointer min-h-87.5"
  >
    ...
    <div
      v-if="user"
      class="absolute left-4 bottom-2 lg:flex grid gap-1 font-semibold text-primary text-sm justify-between items-center"
    > 
      ...
      <button class="btn btn-xs btn-primary" :disabled="quantity <= 0" v-on:click="onAddToCart">
        <Icon name="lucide:shopping-cart" size="12" /> เพิ่มลงตะกล้า
      </button>
    </div>
    <div
      v-else
