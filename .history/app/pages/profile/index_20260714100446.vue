<template>
            ...
            <div class="mt-5">
              ...
              <div v-else class="grid gap-3 md:grid-cols-2">
                <article
                  ...
                  :key="address.uuid"
                  class="rounded-2xl border border-base-300 bg-base-100 p-4 transition hover:ring-2 hover:ring-primary shadow"
                >
  ...
  <dialog ref="createAddressModal" class="modal">
    <div class="modal-box max-w-3xl p-0">
      ...
      >
        <div v-if="confirmAction === 'create'">
          <h2 class="text-2xl font-bold">เพิ่มที่อยู่จัดส่ง</h2>
          <p class="mt-1 text-sm text-base-content/60">
            กรอกรายละเอียดที่อยู่สำหรับผู้สั่งซื้อ
          </p>
        </div>
        <div v-if="confirmAction === 'edit'">
      ...
      <form
        ...
        @submit.prevent="requestSaveAddress"
      >
        <div class="grid gap-4 sm:grid-cols-2">
          <fieldset class="fieldset">
...
const openEditAddressModal = (address: ShippingAddress) => {
  ...
  createAddressForm.value = toShippingAddressForm(address);
  createAddressModal.value?.showModal();
};
