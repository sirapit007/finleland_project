      ...
      <div class="space-y-4 p-4 sm:p-6 lg:p-8">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div class="flex flex-wrap gap-2">
            <NuxtLink to="/shopping-basket" class="btn btn-outline btn-sm">
              <Icon name="lucide:shopping-cart" size="16" /> ไปตะกร้าสินค้า
            </NuxtLink>
        ...
        <div class="grid gap-6 xl:grid-cols-[minmax(0,420px)_minmax(0,1fr)]">
          <section class="rounded-2xl border border-base-300 bg-base-100 p-5">
            ...
            <form class="mt-5 space-y-4" @submit.prevent="saveProfile">
              ...
              <button class="btn btn-primary w-full" type="submit" :disabled="isSavingProfile">
      ...
      >
        ...
        <div class="modal-action">
          ...
          <button class="btn btn-primary flex-1" type="submit" :disabled="isSavingAddress">
            <span v-if="isSavingAddress" class="loading loading-spinner loading-xs" />
...
import type { ShippingAddress } from "~~/composables/useShippingAddresses";
...
const confirmMessage = computed(() => {
  ...
  if (confirmAction.value === "create") return "ตรวจสอบข้อมูลแล้วบันทึกที่อยู่จัดส่งนี้";
  return "ยืนยันการบันทึกการแก้ไขที่อยู่จัดส่งนี้";
});

const confirmButtonText = computed(() => (confirmAction.value === "remove" ? "ลบที่อยู่" : "บันทึก"));
...
const saveRemoveAddress = async () => {
  ...
  try {
    await deleteShippingAddress(removeAddressTarget.value.uuid, currentUser.value);
