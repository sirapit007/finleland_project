<template>
  <div class="w-full bg-base-100">
    <div class="mx-auto w-full max-w-8xl px-0 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        >
              ...
              <table class="table min-w-190 border-separate border-spacing-0">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td
                      class="border-t border-base-300 px-4 py-4 text-right font-bold text-primary"
                    >
                      ฿{{ formatPrice(unitPrice(basket)) }}
                    </td>
                    ...
                    <td
                      class="border-t border-base-300 px-5 py-4 text-right text-lg font-bold text-primary"
                    >
                      ฿{{ formatPrice(basket.basket_total) }}
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          >
            ...
            <div class="mt-5 rounded-xl bg-base-200/50 p-4">
              ...
              <button
                class="btn btn-outline btn-xs mt-4 w-full"
                @click="openSelectAddressModal"
              >
                เลือกที่อยู่จัดส่ง
              </button>
              ...
              >
                ...
                >
                  <Icon
                    :name="option.icon"
                    size="23"
                    :class="option.id === 'express' ? 'text-warning' : 'text-primary'"
                  />
            ...
            <button
              ...
              :disabled="subtotal < 1500 || !selectedShippingAddress"
            ...
            <p
              v-else-if="!selectedShippingAddress"
              class="mt-2 text-center text-xs text-base-content/55"
            >
    ...
    <div class="modal-box max-w-3xl p-0">
      ...
      <div class="max-h-[78vh] space-y-5 overflow-y-auto p-5 sm:p-6">
        <div class="flex flex-wrap justify-between gap-2">
          <button class="btn btn-primary btn-sm" @click="openCreateAddressModal">
  ...
  <dialog ref="createAddressModal" class="modal">
    <div class="modal-box max-w-3xl p-0">
      ...
      <form class="max-h-[78vh] space-y-4 overflow-y-auto p-5 sm:p-6" @submit.prevent="requestCreateAddress">
        ...
        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <label class="form-control">
            <span class="label-text mb-1 text-sm">แขวง / ตำบล</span>
            <input
              v-model.trim="createAddressForm.shipping_subdistrict"
              class="input input-bordered w-full"
              placeholder="คลองตันเหนือ"
            />
          </label>
          <label class="form-control">
            <span class="label-text mb-1 text-sm">เขต / อำเภอ</span>
            <input
              v-model.trim="createAddressForm.shipping_district"
              class="input input-bordered w-full"
              placeholder="วัฒนา"
            />
          </label>
          <label class="form-control">
            ...
            <input
              v-model.trim="createAddressForm.shipping_province"
              class="input input-bordered w-full"
        ...
        <label class="flex cursor-pointer items-center gap-3 rounded-xl border border-base-300 p-4">
          ...
          <div>
            ...
            <p class="text-sm text-base-content/55">
              ระบบจะใช้รายการนี้เป็นค่าเริ่มต้นเมื่อสั่งซื้อ
            </p>
          </div>
        </label>
        ...
        <div class="modal-action">
          ...
          <button class="btn btn-primary flex-1" type="submit" :disabled="isSavingAddress">
            <span
    ...
    <div class="modal-box max-w-3xl p-0">
      ...
      >
        <div>
          ...
          <p class="mt-1 text-sm text-base-content/60">
            ปรับข้อมูลที่อยู่จัดส่งให้เป็นปัจจุบัน
          </p>
        </div>
        <button
          class="btn btn-circle btn-ghost btn-sm"
      ...
      <form class="max-h-[78vh] space-y-4 overflow-y-auto p-5 sm:p-6" @submit.prevent="requestEditAddress">
        <div class="grid gap-3 sm:grid-cols-2">
          <label class="form-control">
            <span class="label-text mb-1 text-sm">ชื่อที่อยู่</span>
            <input
        ...
        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <label class="form-control">
            <span class="label-text mb-1 text-sm">แขวง / ตำบล</span>
            <input
              v-model.trim="editAddressForm.shipping_subdistrict"
              class="input input-bordered w-full"
            />
          </label>
          <label class="form-control">
            <span class="label-text mb-1 text-sm">เขต / อำเภอ</span>
            <input
              v-model.trim="editAddressForm.shipping_district"
              class="input input-bordered w-full"
            />
          </label>
          <label class="form-control">
            <span class="label-text mb-1 text-sm">จังหวัด</span>
            <input
              v-model.trim="editAddressForm.shipping_province"
              class="input input-bordered w-full"
            />
          </label>
        </div>
...
const confirmAction = ref<"remove" | "clear" | "checkout" | "create-address" | "edit-address" | "">("");
...
const confirmMessage = computed(() => {
  ...
  if (confirmAction.value === "clear") return "สินค้าทุกรายการจะถูกลบออกจากตะกร้า";
  if (confirmAction.value === "checkout") return `ยอดสั่งซื้อ ฿${formatPrice(grandTotal.value)} จะถูกส่งให้ร้านตรวจสอบ`;
  if (confirmAction.value === "create-address") return "ตรวจสอบข้อมูลแล้วบันทึกที่อยู่จัดส่งนี้";
  return "ยืนยันการบันทึกการแก้ไขที่อยู่จัดส่งนี้";
});

const confirmButtonText = computed(() => {
  if (confirmAction.value === "remove" || confirmAction.value === "clear") return "ลบ";
  if (confirmAction.value === "checkout") return "ยืนยันการสั่งซื้อ";
  return "บันทึก";
});

const confirmVariant = computed<"error" | "primary">(() =>
  confirmAction.value === "remove" || confirmAction.value === "clear" ? "error" : "primary",
);

const isConfirmLoading = computed(() =>
  isClearing.value || isSavingAddress.value || Boolean(confirmBasketTarget.value && isItemUpdating(confirmBasketTarget.value.uuid)),
);
...
const pickSelectedShippingAddress = () => {
  ...
  selectedShippingAddressId.value =
    shippingAddresses.value.find((address) => address.shipping_is_default)
      ?.uuid || shippingAddresses.value[0]?.uuid || "";
};
...
const loadShippingAddresses = async () => {
  ...
  try {
    shippingAddresses.value = await fetchShippingAddresses(currentUser.value.uuid);
...
const confirmBasketAction = async () => {
  if (confirmAction.value === "remove" && confirmBasketTarget.value) {
    ...
    showToast("ยืนยันคำสั่งซื้อแล้ว ร้านจะติดต่อกลับเพื่อดำเนินการต่อ");
  }
