  ...
  <article class="quotation-sheet">
    <header class="quotation-header">
      ...
      <div class="document-heading">
        <p class="document-title">ใบเสนอราคา</p>
        <p class="document-subtitle">QUOTATION</p>
        <span class="document-badge">{{ quotationNumber }}</span>
      </div>
    </header>
    ...
    <section class="party-grid">
      <div class="party-box">
        <p class="section-label">เสนอราคาแก่</p>
    ...
    <section class="totals-section">
      <div class="amount-words">
        ...
        <div class="order-note">
          <p>
            <strong>เงื่อนไข:</strong> ใบเสนอราคานี้มีอายุ 7
            วันนับจากวันที่ออกเอกสาร
          </p>
          <p>กรุณาติดต่อร้านค้าเพื่อยืนยันสินค้าและกำหนดส่งมอบ</p>
          <p>เอกสารนี้ไม่ใช่ใบเสร็จรับเงินหรือใบกำกับภาษี</p>
    ...
    <section class="signature-grid">
      <div>
        ...
        <p>ผู้เสนอราคา / ผู้มีอำนาจลงนาม</p>
        <p>วันที่ ______ / ______ / ______</p>
      </div>
      <div>
        <span class="signature-line" />
        <p>ผู้ยอมรับใบเสนอราคา</p>
        <p>วันที่ ______ / ______ / ______</p>
      </div>
    </section>
    ...
    <footer class="quotation-footer">
      <p>
        ใบเสนอราคานี้จัดทำจากระบบอิเล็กทรอนิกส์ · จัดทำโดย
        {{ preparedBy || "ผู้ดูแลระบบ" }}
      </p>
    </footer>
  </article>
</template>
...
const quotationNumber = computed(() => {
  const orderNumber = text(order.value.order_number, "");
  return orderNumber
    ? orderNumber.replace(/^ORD(?=-)/i, "QT")
    : `QT-${text(order.value.uuid).slice(0, 8).toUpperCase()}`;
});
const issueDate = computed(
  () =>
    props.printedAt || order.value.order_placed_at || order.value.created_at,
);
const validUntil = computed(() => {
  const date = new Date(String(issueDate.value || ""));
  if (Number.isNaN(date.getTime())) return "";
  date.setDate(date.getDate() + 7);
  return date.toISOString();
});
const taxDetail = computed<QuotationRow | null>(() =>
...
.quotation-sheet > :not(.demo-watermark) {
  position: relative;
  z-index: 1;
}
