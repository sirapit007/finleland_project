...
export default defineEventHandler(async (event) => {
  ...
  const result = await db.query(
    `SELECT 
      base.*,
      user_c.firstname || ' ' || user_c.lastname AS created_username,
      user_u.firstname || ' ' || user_u.lastname AS updated_username,
      user_d.firstname || ' ' || user_d.lastname AS deleted_username
