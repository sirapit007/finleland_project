...
export default defineEventHandler(async (event) => {
  const tableName = "vw_event_promotions";
  ...
  condition += query?.q
    ? ` AND (base.promotion_name ILIKE '%${query?.q}%' OR base.promotion_description ILIKE '%${query?.q}%') `
    : "";

  console.log(now);
  if (now) {
    condition += ` AND base.deleted_at IS NULL AND base.promotion_is_active = true `;
    condition += `
    AND now() BETWEEN
      base.promotion_start_date
      AND
      base.promotion_end_date
  `;
  }
